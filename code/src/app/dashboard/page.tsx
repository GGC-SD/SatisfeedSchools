...
export default function Dashboard() {
  ...
  const [selectedTimeCreated, setSelectedTimeCreated] = useState<Date | null>(
    null
  );
  ...
  return (
    <Container fluid className="p-0">
      <Row className="m-0">
        ...
        <Col md={9} lg={10} className="pt-5 pb-2 d-flex flex-column min-vh-100">
              ...
              <>
                {versionData?.dataset_info && (
                  <p className="text-center text-sm text-gray-600 mb-4">
                    Data collected from{" "}
                    <b>{versionData.dataset_info.startDate}</b> to
                    <b>{versionData.dataset_info.endDate}</b> —{" "}
                    {versionData.dataset_info.recordCount} records
                  </p>
                )}
