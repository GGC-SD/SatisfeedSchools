...
import { replaceFixedRadiusFromCenter, clearFixedRadius, PolygonFeature } from "./click-radius";
 ...
 */
export default function SchoolsClusterOverlay({ map, idSuffix = "", onAreaSelect }: Props) {
  useEffect(() => {
    if (!map) return;
    const m = map;
    ...
    async function draw() {
      ...
      const onPointClick = (e: MapLayerMouseEvent) => {
        ...
        const poly = replaceFixedRadiusFromCenter(m, [lng, lat], 3, { idSuffix });
